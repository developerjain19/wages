                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <?php if ($tag == 'edit') {
                                    if ($this->delete == '1') {
                                ?>
                                        <a href="<?php echo base_url() . 'incentive-range-list?BdID=' . encryptId($resource[0]['id']) ?>" class="btn btn-danger shadow btn-xs sharp is_permission" onclick="return confirm('Are you sure to delete this data?')"><i class="fa fa-trash"></i></a>
                                    <?php }
                                }
                            
                                else { ?>
                                    <a href="<?= base_url('incentive-range-list') ?>" class="btn btn-success btn-sm">Incentive List <i class="fa fa-list"></i></a>
                                <?php
                                }
                                ?>
                            </div>
        ...
        <script>
            $('.submit_form').click(function(e) {
                ...
                if (isEmpty == true) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '<?php echo base_url('Admin_Dashboard/edit_incentive') ?>',
                        data: {
                            min_qty: min_qty,
                            max_qty: max_qty,
                            amount: amount,
                        ...
                        success: function(response) {
                          ...
                          window.location.href="<?= base_url('incentive-range-list') ?>"
            ...
            $(document).on("click", ".delete_exp_btn", function(e) {
                ...
                if (confirm("Do you really want to delete this record ?")) {
                    ...
                    $.ajax({
                        url: "<?= base_url('Admin_Dashboard/delete_incentive') ?>",
                        type: "POST",
                        data: {
                            expId: expId
                        },
